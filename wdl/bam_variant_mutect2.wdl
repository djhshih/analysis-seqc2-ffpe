...
task intervals_split {
    ...
    command {
        ...
        gatk --java-options "-Xmx~{runtime_params.command_mem}m -XX:+UseSerialGC" SplitIntervals \
            -R ~{ref_fasta} \
            ~{"-L " + intervals} \
...
task bam_mutect2 {
    ...
    command <<<
        ...
        gatk --java-options "-Xmx~{command_mem}m -XX:+UseSerialGC" GetSampleName -R ~{ref_fasta} -I ~{tumor_bam} -O tumor_name.txt -encode
        tumor_command_line="-I ~{tumor_bam} -tumor `cat tumor_name.txt`"

        if [[ ! -z "~{normal_bam}" ]]; then
            gatk --java-options "-Xmx~{command_mem}m -XX:+UseSerialGC" GetSampleName -R ~{ref_fasta} -I ~{normal_bam} -O normal_name.txt -encode
        ...
        if [[ ! -z "~{variants_for_contamination}" ]]; then
            gatk --java-options "-Xmx~{command_mem}m -XX:+UseSerialGC" GetPileupSummaries -R ~{ref_fasta} -I ~{tumor_bam} ~{"--interval-set-rule INTERSECTION -L " + intervals} \
                -V ~{variants_for_contamination} -L ~{variants_for_contamination} -O tumor-pileups.table

            if [[ ! -z "~{normal_bam}" ]]; then
                gatk --java-options "-Xmx~{command_mem}m -XX:+UseSerialGC" GetPileupSummaries -R ~{ref_fasta} -I ~{normal_bam} ~{"--interval-set-rule INTERSECTION -L " + intervals} \
...
task orientation_model_learn {
    ...
    command {
        ...
        gatk --java-options "-Xmx~{command_mem}m -XX:+UseSerialGC" LearnReadOrientationModel \
            -I ~{sep=" -I " f1r2_tar_gz} \
...
task bam_outs_merge {
    ...
    command <<<
        ...
        set -e
        export GATK_LOCAL_JAR=~{default="/root/gatk.jar" runtime_params.gatk_override}
        gatk --java-options "-Xmx~{runtime_params.command_mem}m -XX:+UseSerialGC" GatherBamFiles \
        ...
        gatk --java-options "-Xmx~{runtime_params.command_mem}m -XX:+UseSerialGC" SortSam -I unsorted.out.bam \
            -O ~{output_vcf_name}.out.bam \
            --SORT_ORDER coordinate -VALIDATION_STRINGENCY LENIENT
        gatk --java-options "-Xmx~{runtime_params.command_mem}m -XX:+UseSerialGC" BuildBamIndex -I ~{output_vcf_name}.out.bam -VALIDATION_STRINGENCY LENIENT
    >>>
...
task pileup_summaries_merge {
    ...
    command {
        ...
        gatk --java-options "-Xmx~{runtime_params.command_mem}m -XX:+UseSerialGC" GatherPileupSummaries \
...
task pileups_calculate_contamination {
    input {
      String? intervals
      File tumor_pileups
      File? normal_pileups
    ...
    command {
        ...
        gatk --java-options "-Xmx~{runtime_params.command_mem}m -XX:+UseSerialGC" CalculateContamination -I ~{tumor_pileups} \
...
task vcfs_merge {
    ...
    command {
        set -e
        export GATK_LOCAL_JAR=~{default="/root/gatk.jar" runtime_params.gatk_override}
        gatk --java-options "-Xmx~{runtime_params.command_mem}m -XX:+UseSerialGC" MergeVcfs -I ~{sep=' -I ' input_vcfs} -O ~{output_vcf}
    }
...
task stats_merge {
    ...
    command {
        ...
        gatk --java-options "-Xmx~{runtime_params.command_mem}m -XX:+UseSerialGC" MergeMutectStats \
