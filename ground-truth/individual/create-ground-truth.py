...
def process_dataset(dataset: str, variant_set: str, vcf_suffix: str = ".vcf.gz") -> None:
	...
	## Declare Fresh Frozen dataset and variant set name
	ff_set = variant_set
	ff_dataset = {
		"FFX" : "WES",
		"FFG" : "WGS"
	...
	ff_paths = sorted(
		glob.glob(f"{repo_root}/vcf/{ff_dataset[dataset]}/{ff_set}/*/*.vcf.gz")
	)
	...
	for i, path in enumerate(ffpe_paths, start=1):
		...
		outdir = f"{repo_root}/ground-truth/individual/{dataset}/{variant_set}/{ffpe_sample_name}"
