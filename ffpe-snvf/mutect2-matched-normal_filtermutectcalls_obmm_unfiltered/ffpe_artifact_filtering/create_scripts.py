...
def return_path_if_exist(path: str) -> str:
...
vcf_paths = glob.glob("../../../vcf/mutect2-matched-normal_filtermutectcalls_obmm_unfiltered/*/*/*.vcf")
...
for i in range(bam_vcf_table.shape[0]):
	...
	for model in models:
		...
		filter_script_path = return_path_if_exist(f"../../{model}.sh.template")
