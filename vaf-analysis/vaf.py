...
# %%
def process_vcf(vcf_path: str, sample_name: str, normal_name: str, snv_only = True) -> pl.DataFrame:
...
# %%
# Take variant AF data, plot and describe
def perform_analysis(df: pl.DataFrame, vcf_type: str, sample_name: str, outdir: str) -> None:
	
	if vcf_type == "matched-normal":
		cols = ['chrom', 'pos', 'id', 'ref', 'alt', 'tumor.AF', 'tumor.AD', 'normal.AF', 'normal.AD', 'tumor.AD_derived_VAF']
	else:
		cols = ['chrom', 'pos', 'id', 'ref', 'alt', 'tumor.AF',  'tumor.AD', 'tumor.AD_derived_VAF']
...
for i, sample_name in enumerate(vcf_table["sample_name"]):
	...
	matched = process_vcf(vcf_table[i, "matched_normal_vcf_path"], sample_name, normal_name)
	...
	tumor_only = process_vcf(vcf_table[i, "tumor_only_vcf_path"], sample_name, normal_name)
	...
	matched = matched.with_columns(pl.lit(sample_name).alias("sample_name"))
	tumor_only = tumor_only.with_columns(pl.lit(sample_name).alias("sample_name"))

	matched_normal_combined.append(matched)
	tumor_only_combined.append(tumor_only)

matched_normal_combined = pl.concat(matched_normal_combined)
tumor_only_combined = pl.concat(tumor_only_combined)
