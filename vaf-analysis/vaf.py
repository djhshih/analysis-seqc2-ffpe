...
def process_vcf(vcf_path: str, vcf_type: str, sample_name: str, normal_name: str = "", snv_only = True) -> pl.DataFrame:
	...
	df = df.with_columns((pl.col("tumor.AD").list.get(1) / (pl.col("tumor.AD").list.get(0) + pl.col("tumor.AD").list.get(1))).alias("tumor.VAF"))
 
	if vcf_type == "matched-normal":
		cols = ['sample_name', 'chrom', 'pos', 'ref', 'alt', 'tumor.AF', 'tumor.AD', 'normal.AF', 'normal.AD', 'tumor.VAF']
	else:
		cols = ['sample_name', 'chrom', 'pos', 'ref', 'alt', 'tumor.AF',  'tumor.AD', 'tumor.VAF']
...
def perform_analysis(df: pl.DataFrame, vcf_type: str, sample_name: str, outdir: str, write: bool = True) -> tuple[pl.DataFrame, pl.DataFrame]:
	...
	stats_vaf = df[f"tumor.VAF"].describe()
	if write:
		stats_vaf.write_csv(f"{outdir}/{sample_name}.{vcf_type}.VAF.stats.tsv", separator="\t")
	...
	if write:
		...
		# plt.close()
		sns.displot(data=df, x=f"tumor.VAF", bins = 100).savefig(f"{outdir}/{sample_name}.{vcf_type}.VAF.pdf")
		plt.close()
	else:
		# sns.displot(data=df, x=f"tumor.AF", bins = 100)
		sns.displot(data=df, x=f"tumor.VAF", bins = 100)
...
for dataset in datasets:
	...
	filter_type = "pass-orientation-dp20-filtered"
	outdir_root = f"./{filter_type}/{dataset}"

	matched_vcf_paths = glob.glob(f"../vcf/mutect2-matched-normal_{filter_type}/{dataset}/*/*.vcf")
	tumor_only_vcf_paths = glob.glob(f"../vcf/mutect2-tumor-only_{filter_type}/{dataset}/*/*.vcf")
	...
	for i, sample_name in enumerate(vcf_table["sample_name"]):
		...
		stats_vaf = perform_analysis(matched, "matched-normal", sample_name, outdir)
		...
		stats_vaf = perform_analysis(tumor_only, "tumor-only", sample_name, outdir)
	...
	matched_stats_vaf.write_csv(f"{outdir_root}/per-{dataset.lower()}-samples.matched-normal.VAF.stats.tsv", separator="\t")
	tumor_only_stats_vaf.write_csv(f"{outdir_root}/per-{dataset.lower()}-samples.tumor-only.VAF.stats.tsv", separator="\t")

	_ = perform_analysis(matched_normal_combined, "matched-normal", f"all-{dataset.lower()}-samples", outdir_root)
	_ = perform_analysis(tumor_only_combined, "tumor-only", f"all-{dataset.lower()}-samples", outdir_root)
