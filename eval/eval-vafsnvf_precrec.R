...
## Setup
repo_root <- ".."
eval_dir <- file.path(repo_root, "eval")
...
evaluate_sample_set <- function(
	...
	for (i in seq_along(snvf_paths)){
		...
		ground_truth_path <- file.path("..", "ground-truth", "individual", dataset, gsub("-micr1234", "", variant_set), sample_name, sprintf("%s.ground-truth.tsv", sample_name))
