...
#' @param vcf_dir (string) | Directory containing the VCFs
...
process_samples <- function(ffpe_snvf_dir, vcf_dir, model_name, ground_truth_variants, outdir_root) {
	message("Processing samples with a unified ground truth:")

	# Construct the search path to find all of the model's result files
	search_path <- file.path(ffpe_snvf_dir, unlist(strsplit(model_name, "-"))[1], "*", sprintf("*.%s.tsv", model_name))
	...
	for (path in model_result_paths) {
		...
		vcf_path = Sys.glob(file.path(vcf_dir, sample_name, sprintf("%s*.vcf", sample_name)))[1]
		# print(vcf_path)
...
evaluate_dataset <- function(
	...
	## Directory for the FFPE VCFs
	vcf_dir <- file.path(vcf_dir, dataset_id)
	...
	# Use High Confidence Ground Truth Variants provided by seqc2
	message("Using SEQC2 Ground Truth Variants for evaluation.")
	gt_variants <- read.delim(file.path("ground-truth", "seqc2_ground-truth_snv_pass-high-med-conf.tsv"))
	gt_variants <- add_id(gt_variants)
	...
	process_samples(
		...
		ground_truth_variants = gt_variants, 
...
# Filtered FFX dataset vs Filtered WES VCF, DP >= 10, Exome Target, High Conf Region, Blacklist filtered; MICR1234 removed
evaluate_dataset(
	dataset_id = "FFX",
	vcf_dir = "../vcf/mutect2-matched-normal_exome-hc-target_pass-orientation-dp-blacklist-filtered",
	ffpe_snvf_dir = "../ffpe-snvf/mutect2-matched-normal_exome-hc-target_pass-orientation-dp-blacklist-filtered_micr1234",
	outdir_root = "mutect2-matched-normal_exome-hc-target_pass-orientation-dp-blacklist-filtered_micr1234",
	model_name = model_name,
...
# FFG dataset vs Filtered WGS VCF, DP>=10, High Conf Region, Blacklist filtered
evaluate_dataset(
	dataset_id = "FFG",
	vcf_dir = "../vcf/mutect2-matched-normal_hc-target_pass-orientation-dp-blacklist-filtered",
	ffpe_snvf_dir = "../ffpe-snvf/mutect2-matched-normal_hc-target_pass-orientation-dp-blacklist-filtered",
	outdir_root = "mutect2-matched-normal_hc-target_pass-orientation-dp-blacklist-filtered",
	model_name = model_name,
